...
int fk_ev_ioev_remove(fk_ioev *ioev)
{
	...
	if (ioev->active == 1) {
		fk_rawlist_remove(evmgr.act_ioev, ioev);
		ioev->active = 0;
	}
...
int fk_ev_tmev_remove(fk_tmev *tmev)
{
	...
	if (tmev->expired == 1) {
		fk_rawlist_remove(evmgr.exp_tmev, tmev);
	}
...
int fk_ev_pending_tmev_update()
{
	...
	while (root != NULL) {
		...
		if (cmp >= 0) {
			...
			fk_rawlist_insert(evmgr.exp_tmev, tmev);//add to the exp list
			tmev->expired = 1;
...
int fk_ev_expired_tmev_proc()
{
	...
	while (tmev != NULL) {
		...
		fk_rawlist_remove(evmgr.exp_tmev, cur);//remove current from the expired list
		cur->expired = 0;
...
int fk_ev_active_ioev_proc()
{
	...
	while (ioev != NULL) {
		...
		fk_rawlist_remove(evmgr.act_ioev, cur);
...
int fk_ev_ioev_activate(int fd, char type)
{
	...
	if (type & FK_IOEV_READ) {
		...
		if (rioev != NULL) {//when EPOLLERR/EPOLLHUP occurs, maybe there is no rioev/wioev, so check non-null
			if (rioev->active == 0) {
				fk_rawlist_insert(evmgr.act_ioev, rioev);//add to the exp list
				rioev->active = 1;
			}
		}
	}
	if (type & FK_IOEV_WRITE) {
		...
		if (wioev != NULL) {
			if (wioev->active == 0) {
				fk_rawlist_insert(evmgr.act_ioev, wioev);//add to the exp list
				wioev->active = 1;
			}
		}
	}
