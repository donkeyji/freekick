...
fk_fkdb_bgsave(void)
{
    ...
    if (rt < 0) { /* should not exit here, just return */
        ...
        /*
         * use _exit() instead of exit() in the child process, since _exit()
         * does not flush the standard I/O buffer. if exit() is used here, maybe
         * the same standard I/O buffer will be flushed both in parent and child
         * process
         */
        _exit(EXIT_SUCCESS);
    }
